...
class PartieLoupGarou:
    ...
    def voter(self, discussion, data):
        ...
        for joueur in self.joueurs:
            ...
            vote_AI(joueur.nom, player_to_kill, why)
        ...
        maxi = max(votes[1])
        nb_votes = votes[1]
        player_to_kill = votes[0][nb_votes.index(maxi)]
...
# @socketio.on('vote_AI')
# def vote_AI(player_name, player_to_kill, why):
#     decision_joueur = player_name + " voted to kill " + player_to_kill + " because " + why + '\n'
#     # Logique pour exécuter une fonction côté client
#     socketio.emit('function_executed', decision_joueur)
    
@socketio.on('vote_AI')
def vote_AI(player_name, player_to_kill, why):
    message = player_name + " voted to kill " + player_to_kill + " because " + why + '\n'
    socketio.send(message)

# @app.route('/vote_AI', methods=['GET'])
# def vote_AI(player_name, player_to_kill, why):
#     decision_joueur = player_name + " voted to kill " + player_to_kill + " because " + why + '\n'
#     # Call the function from your Python script and capture the output
#     output_answer = decision_joueur
