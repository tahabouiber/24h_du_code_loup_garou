...
def choisir_role():
    while True:
        role = input("Choisissez votre rôle (wolf/villager) : ")
        if role.lower() == "wolf" or role.lower() == "villager":
            return role.lower()
        else:
            print("Rôle invalide. Veuillez choisir 'wolf' ou 'villager'.")

def lancer_partie(wolves_nb, villagers_nb):
    # Ask the user to choose their role
    user_role = choisir_role()
